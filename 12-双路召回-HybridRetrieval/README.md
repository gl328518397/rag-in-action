...
RRF 融合与混合检索的完整原理，见配套书稿《现代知识检索系统》**第9章 §9.3 混合检索**；"结构化数据不该压成向量、该走独立通路"的判断，见**第7章 知识图谱与结构化知识源**。本加餐把书稿里 Dense+BM25 的两路，换成了业务库场景更常见的 SQL+Vector 两路——同一个 RRF 思想，落在关系数据库这种结构化载体上。
